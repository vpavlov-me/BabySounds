...
struct ParentGateView: View {
    ...
    private func challengeView(for challenge: ParentGateChallenge) -> some View {
        ...
        case let .mathAddition(a, b, options):
            mathChallengeView(
                question: "\(a) + \(b) = ?",
        ...
        case let .mathSubtraction(a, b, options):
            mathChallengeView(
                question: "\(a) - \(b) = ?",
                options: options.map(String.init),
                correctAnswer: String(a - b)
        ...
        case let .readingChallenge(word, options):
            readingChallengeView(
                word: word,
                options: options
            )

        case let .timeChallenge(hour, minute, options):
            timeChallengeView(
                hour: hour,
                minute: minute,
                options: options
        ...
        case let .textInput(question, answer):
    ...
    private func textInputChallengeView(question: String, expectedAnswer _: String) -> some View {
        VStack(spacing: 24) {
            Text(question)
                .font(.title3)
                .fontWeight(.medium)
    ...
    private func answerButton(option: String, correctAnswer _: String) -> some View {
        Button(action: {
            selectAnswer(option)
        }) {
            Text(option)
