...
public class SafeVolumeManager: ObservableObject {
    ...
    private init() {
        // Load saved settings
        isSafeVolumeEnabled = UserDefaults.standard.object(forKey: safeVolumeEnabledKey) as? Bool ?? true
        safeVolumeMultiplier = UserDefaults.standard.object(forKey: safeVolumeMultiplierKey) as? Float ?? SafetyLimits
            .defaultChildVolume
        isParentalOverrideActive = UserDefaults.standard.bool(forKey: parentalOverrideKey)
        ...
        {
            lastVolumeWarningTime = lastWarningTime
        }
    ...
    @objc
    private func audioSessionRouteChanged(notification: Notification) {
    ...
    @objc
    private func audioSessionInterrupted(notification _: Notification) {
...
// MARK: - SafeVolumeManager.VolumeWarningLevel + RawRepresentable
